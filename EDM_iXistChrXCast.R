...
# 1. An allelic ratio table, where columns from 7 onwards are samples of the ES-to-NPC differentiation timecourse
...
### SET-UP OF MODEL (for iXist-ChrX-Cast) ####
AR_table_forEDM <- C7H8_AR_table #this is an allelic ratio table from 2/3 replicates of ES-to-NPC differentiation timecourse in iXist-ChrX-Cast cells
NPC_escapees <- C7H8_NPC_escapees #this is a list of escapee genes in iXist-ChrX-Cast cells, defined as genes with an allelic ratio > 0.1 in an average of two NPC chrRNA-seq samples
AR_table_forEDM_merged <- C7H8_AR_table_merged #this the allelic ratio table with replicates of the same timepoint averaged together

options(scipen=999)

# From column names, collect timepoints of samples in the allelic ratio table
colnames(AR_table_forEDM)
colnames(AR_table_forEDM_merged)
timepoints <- c(0,0,24,24,48,48,72,72,72,120,120,120,168,168,168,360,504)
...
#simple boxplot all samples to input into model, check agree  with given timpeoints
...
#compare boxplots real data to model
par(mfrow=c(1,2))
...
boxplot.matrix(EDM_Fit_Matrix[,c(1,3,5,7,10,13,16)], use.cols=TRUE,
               ylab = 'chrRNA Allelic Ratio (Xi/(Xi+Xa))',
               xlab = 'model data',
               ylim=c(0,1))
abline(h=0.5)
#summary values and density of halftimes (hours)
